...
This event can be triggered by using the SDK or from a smart contract.

You can inspect a uniswap transaction to see the details of the transaction but the general pattern of a swap is as follows:
 - Approve token allowance (if swapping Token-> ETH or Token -> Token).
 - Calculate prices and format a `swap` event with the relevant data handled by the Uniswap Router.
 -- Router swaps ETH -> WETH if needed
 -- Router sends tokens to the tokens output pool and receives the result so long as it falls within the bounds set
 -- Router swaps WETH -> ETH if needed
 -- Router returns output tokens to the 
